...
watch([searchName, searchStatus], ([newSearchName, newSearchStatus]) => {
    const searchParams = {};
    if (newSearchName) {
        searchParams.nombre = newSearchName;
    }
    if (newSearchStatus) {
        searchParams.estado = newSearchStatus;
    }
...
const loadPermisos = async () => {
    try {
        const response = await axios.get(PERMISOS_URL);
...
const editRole = (role) => {
    form.value = {
        ...
        permisos: role.permisos
            ? role.permisos.map((permiso) => permiso.id)
            : [],
    };
...
// Cargar los roles y permisos cuando se monta el componente
onMounted(async () => {
    await loadPermisos();
    await loadRoles();
});
